...
//		System.out.println(list);
	...
	public void updatecookcode(Integer cookquantityinbasket, Integer usercode, Integer cookcode) {
		System.out.println("===============" + searchDao.findbasket(usercode, cookcode));
		if (searchDao.findbasket(usercode, cookcode).isEmpty()) {
			System.out.println("장바구니에 신규 데이터를 추가해야 함");
			searchDao.getcookbasket(cookquantityinbasket, usercode, cookcode);
		} else {
			System.out.println("장바구니 기존 데이터를 변경해야 함");
...
//		System.out.println("cookbasketdata" + cookbasketdata);
		...
		for (int i=0; i<cookbasketdata.size(); i++) {
			System.out.println(i+"번째 내용 "+cookbasketdata.get(i));
